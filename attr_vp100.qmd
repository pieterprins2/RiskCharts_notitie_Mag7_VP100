...
De VP100 laat een mooie waarde-ontwikkeling zien. Vanaf de start in juni 2017 tot juni 2025 een bruto performance van `r str_c(round(perf_VP100*100,1), "%")`. De performance van de MSDEWIN in die periode is `r str_c(round(perf_MSDEWIN*100,1), "%")`. Er is eerst sprake van outperformance, daarna van underperformance.
...
Over de hele periode is sprake van een underperformance ("geometrisch" berekend) van `r str_c(-round(100*((1 + perf_VP100)/(1+perf_MSDEWIN) - 1), 1), "%")`.
...
# data_sd_hele_periode |>
#   ggplot(aes(x = pf_BM, y = sd, fill = pf_BM, alpha = pf_BM)) + 
#   geom_col() + 
#   scale_fill_manual(values = c(VP100green, VP100green, bmgray)) +
#   scale_alpha_manual(values = c(1, 0.5, 1)) +
#   scale_y_continuous(labels = percent_format(), sec.axis = dup_axis()) +
#   geom_text(aes(label = str_c(round(sd * 100, 1), "%")), col = "gray30", size = 4, fontface = "bold", position = position_stack(vjust = 0.5)) + 
#   labs(x = "", y = "",
#        title = "standaarddeviatie jul-2017 - jun-2025, VP100, portefeuille in- en exclusief cash vs. MSDEWIN") +
#   charts_custom_theme +
#   theme_minimal() +
...
### Attributie van het verschil met de benchmark
In de volgende figuur kun je zien dat per saldo de asset allocatie de grootste negatieve factor was (de portefeuille is niet vol belegd geweest terwijl de markt is gestegen. Het resultaat RD is het verschil tussen de performance van het RD-deel van de portefeuille en de MSDEWIN. In de eerste periode was dit sterk positief en in de tweede periode sterk negatief.
...
Ter illustratie hieronder het gewicht van cash en obligaties in de VP100. In de eerste periode was er een relatief hoog gewicht in cash en obligaties, in de tweede periode is dat fors gedaald. De portefeuille is dus niet vol belegd geweest.
...
### Attributie van het resultaat RD
...
all_prices_eur_long |> 
  ...
  labs(x = "", y = "",
       title = "price performance in euro's, twee periodes, Mag7 en MSDEWIN") +
  charts_custom_theme +
  theme_minimal() +
  theme(legend.position = "bottom",
        legend.title = element_blank(),
...
Om in zo’n scenario, ervan uitgaande dat je van tevoren niet weet welke die Mag7 zijn, de index bij te houden (een lage tracking error) zul je er niet 15-30 moeten hebben maar eerder honderden. Simulatie over een lange periode, universum is ACWI.
